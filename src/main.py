...
def renderA3C(**kwargs):
    if kwargs['load_file'] is None:
        parameters_file = get_default_save_filename(kwargs['episodes'], kwargs['discount'], kwargs['tmax'],
                                                    kwargs['actor_lr'], kwargs['critic_lr'])
    else:
...
def main():
    ...
    parser.add_argument('--tmax', help='Max steps before update.', type=int, default=5)
